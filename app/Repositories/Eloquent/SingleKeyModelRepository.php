...
class SingleKeyModelRepository extends BaseRepository implements SingleKeyModelRepositoryInterface
{
    ...
    public function paginate($page_number = 5, $where_condition = null)
    {
        $modelClass = $this->getModelClassName();
        if (isset($where_condition)) {

            $model = $modelClass::where($where_condition)->paginate($page_number);
        } else {
            $model = $modelClass::paginate($page_number);
        }
